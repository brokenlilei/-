...
jquery = require('jquery'),    //node_moudels里面
fs = require('fs'),    
urlModel = require('url'),     //能够把URL分割为路径，查询信息，hash值。。。。
...
*/
 const app = http.createServer((req,res)=>{
     ...
     const {pathname,query} = urlModel.parse(rel.url);
                        ...
                        if(err.code === 'EEXIST'){      //说明重名了
                            fs.readdir('www',(error,fileAry)=>{
                                ...
                                fs.mkdir('www/'+name+'/',(err)=>{
                                    ...
                                    res.end(JSON.stringify({code:1,mdg:'创建文件夹成功'}));
                                });
                            });
                        }
                        console.log('创建失败');
